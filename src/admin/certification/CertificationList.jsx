...
const CertificationList = ({ isLoading, data, refetch }) => {
  ...
  const columns = [
    ...
    columnHelper.accessor((row) => row.title, {
      ...
      cell: (info) => (
        <p className="whitespace-nowrap fw-500">{info.getValue()}</p>
      ),
      header: (info) => info.column.id,
    }),
    columnHelper.accessor((row) => row.description, {
      id: "Description",
      cell: (info) => (
        <p className="whitespace-normal w-[200px] fw-500">{info.getValue()}</p>
      ),
      header: (info) => info.column.id,
    }),
        ...
        <>
          <Menu placement="bottom-end">
            ...
            <MenuList className="">
              ...
              >
                <BsArrowsExpand /> Awarded Members
              </MenuItem>
              <MenuItem
                className="my-1 fw-500 flex items-center gap-x-2 pt-1"
                onClick={() => handleOpenEdit(info.row.original)}
              >
                <BiEdit /> Edit Certification
              </MenuItem>
              <MenuItem
  ...
  return (
    <div>
      ...
      <Edit title={'Edit Certification Details'}>
        <EditCertification item={selectedItem} close={() => ShowEdit(false)} refetch={refetch}/>
      </Edit>
    </div>
  );
};
